...
describe IaHarvester, :webmock => true do
  ...
  let(:search_page_1_url) { base_search_url + collection_qs + '&size=1000' }
  let(:search_page_2_url) { base_search_url + collection_qs + '&size=1000&cursor=W3siaWRlbnRpZmllciI6ImFkY2xlbWVudGVteGlpcG8wMHBpY2MifV0=' }
  ...
  let(:search_page_1_response) do
    <<-EOS
{"items":[{"identifier":"id1"},{"identifier":"id2"}],"count":2,"cursor":"W3siaWRlbnRpZmllciI6ImFkY2xlbWVudGVteGlpcG8wMHBpY2MifV0=","total":4}
    EOS
  end

  let(:search_page_2_response) do
    <<-EOS
{"items":[{"identifier":"id3"},{"identifier":"id4"}],"count":2,"total":4}
    EOS
  end
